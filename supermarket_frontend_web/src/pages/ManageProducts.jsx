...
const ManageProducts = () => {
  ...
  const handleSuggestSubmit = async (e) => {
    e.preventDefault();
    if (
      !suggestForm.ProductName ||
      !suggestForm.Description ||
      !suggestForm.SuggestedPrice
    ) {
  ...
  const fetchGovProducts = async () => {
    ...
    if (!GOV_BACKEND_URL) {
      console.error(
        "REACT_APP_GOV_BACKEND_URL is not defined. Using empty GOV products."
      );
      setGovProducts([]);
      return;
    }
  ...
  useEffect(() => {
    const mappedShelves = relations.map((rel) => {
      ...
      const lastEditDate = rel.ChangedAt
        ? new Date(rel.ChangedAt).toLocaleDateString("en-US", {
            year: "numeric",
            month: "2-digit",
            day: "2-digit",
          })
  ...
  const handleAddSelectedProducts = async () => {
    ...
    for (const productId of selectedGovProductIDs) {
      ...
      if (
        ...
        alert(
          `Please enter a valid positive price for product ID ${productId}.`
        );
        return;
      }
    }
  ...
  const handleConfirmChangeShelfProduct = async (shelfId, newProductName) => {
    ...
    if (!trimmedName) {
      alert(
        "Product name cannot be empty. To empty the shelf, use the 'Empty Shelf' button or assign a product."
      );
  ...
  const handleRemoveProductFromShelf = async (shelfId) => {
    if (
      !window.confirm(
        "Are you sure you want to remove the product from this shelf? The shelf will be marked as empty."
      )
    ) {
      return;
    }
    ...
    <tr key={`${shelf.shelfId}-${shelf.productId || "empty"}`}>
      ...
      <td>
        <input
          ...
          placeholder={
            shelf.productName === "Empty" ? "Type product name & blur" : ""
          }
          onBlur={(e) =>
            handleConfirmChangeShelfProduct(shelf.shelfId, e.target.value)
          }
        />
      </td>
    ...
    <>
      ...
      <div className="manage-container">
        ...
        <main className="manage-content">
            ...
            <div className="store-products-layout">
                      ...
                      {products.length === 0 ? (
                        ...
                        products.map((product) => (
                          <tr key={product.ProductID}>
                            ...
                            <td>
                              ₺{product.SuggestedPrice?.toFixed(2) || "N/A"}
                            </td>
                            ...
                            <td>
                              ...
                              product.DiscountEndDate
                                ? `${product.Discount}% until ${new Date(
                                    product.DiscountEndDate
              ...
              <div className="store-products-right-col">
                <div className="gov-add-container">
                  ...
                  <p>
                    Select products below to add to your store. Set a price for
                    each. If not listed,{" "}
                    ...
                    {availableGovProducts.length > 0 ? (
                      availableGovProducts.map((item) => {
                        ...
                        const isOverThreshold =
                          PriceInfo.Threshold &&
                          customPrice &&
                          parseFloat(customPrice) > PriceInfo.Threshold;
                          ...
                          >
                            ...
                            <p>
                              ...
                              {Category.CategoryName ||
                                categoryNameMap[Category.CategoryID] ||
                                "N/A"}
                            </p>
                            <p className="price-info">
                              Gov Suggested: ₺
                              {PriceInfo.SuggestedPrice?.toFixed(2) || "N/A"} |
                              Threshold: ₺
                              {PriceInfo.Threshold?.toFixed(2) || "N/A"}
                            </p>
                            {isSelected && (
                              <div
                  ...
                  {availableGovProducts.length > 0 &&
                    selectedGovProductIDs.length > 0 && (
                      ...
                      >
                        Add {selectedGovProductIDs.length} Selected Product(s)
                        to Store
                      </button>
                    )}
                </div>
            ...
            <div className="shelf-products-layout">
              <div className="shelf-products-left-col">
                ...
                {displayShelves.length === 0 ? (
                  <p className="placeholder-text">
                    No shelves found or no products assigned to shelves.
                  </p>
                ) : (
                  <div className="table-wrapper">
                    <table className="data-table">
              ...
              <div className="shelf-products-right-col">
                <h2 className="table-title">
                  Products Not Currently on a Shelf
                </h2>
                {unshelvedProducts.length === 0 ? (
                  <p className="placeholder-text">
                    All products are currently on a shelf or no products in
                    store.
                  </p>
                ) : (
                  <div className="table-wrapper">
          ...
          {activeTab === "productPrices" && <ManageProductPricesTable />}
        </main>
      </div>
      <Footer />
      ...
      {showSuggestModal && (
        <div className="suggest-product-modal-overlay">
          <div
            className="suggest-product-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <form onSubmit={handleSuggestSubmit}>
              <div className="suggest-product-form-group">
                <label
                  htmlFor="suggestProductName"
                  className="suggest-product-label"
                >
                  Product Name
                </label>
                <input
              ...
              <div className="suggest-product-form-group">
                <label
                  htmlFor="suggestSuppermarket"
                  className="suggest-product-label"
                >
                  Supermarket Name (Optional)
                </label>
                <input
              ...
              <div className="suggest-product-modal-actions">
                <button
                  ...
                  className="suggest-product-button-cancel"
                >
                  Cancel
                </button>
                <button type="submit" className="suggest-product-button-submit">
                  Send Suggestion
                </button>
              </div>
            </form>
