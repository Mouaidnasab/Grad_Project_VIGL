...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    ...
    async function fetchPenalties() {
      try {
        const res = await api.get("/penalty/get");
        setTotalPenalties(res.data?.length ?? 0);
      } catch (error) {
        console.error("Failed to fetch penalties:", error);
      }
    }

    async function fetchSupermarketsCount() {
      try {
        const res = await api.get("/supermarket/count");
        const count = res.data.count !== undefined ? res.data.count : res.data;
        setSupermarketsCount(count);
      } catch (error) {
        console.error("Failed to fetch supermarkets count:", error);
      }
    }

    fetchProducts();
    fetchPenalties();
    fetchSupermarketsCount();
  }, []);
